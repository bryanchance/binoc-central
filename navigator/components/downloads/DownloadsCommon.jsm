...
var DownloadsCommon = {
  ...
  get strings() {
    ...
    while (enumerator.hasMoreElements()) {
      let string = enumerator.getNext().QueryInterface(Components.interfaces.nsIPropertyElement);
  ...
  openDownloadedFile(aFile, aMimeInfo, aOwnerWindow) {
    ...
    if (aMimeInfo && !(aMimeInfo instanceof Components.interfaces.nsIMIMEInfo)) {
      throw new Error("Invalid value passed for aMimeInfo");
    }
    if (!(aOwnerWindow instanceof Components.interfaces.nsIDOMWindow)) {
  ...
  showDirectory(aDirectory) {
    if (!(aDirectory instanceof Components.interfaces.nsIFile)) {
  ...
  async confirmUnblockDownload({ verdict, window,
    ...
    let buttonFlags =
        (Components.interfaces.nsIPrompt.BUTTON_TITLE_IS_STRING * Components.interfaces.nsIPrompt.BUTTON_POS_0) +
        (Components.interfaces.nsIPrompt.BUTTON_TITLE_CANCEL * Components.interfaces.nsIPrompt.BUTTON_POS_1);

    switch (dialogType) {
      case "unblock":
        // Use only the unblock action. The default is to cancel.
        buttonFlags += Components.interfaces.nsIPrompt.BUTTON_POS_1_DEFAULT;
        break;
      case "chooseUnblock":
        // Use the unblock and remove file actions. The default is remove file.
        buttonFlags +=
          (Components.interfaces.nsIPrompt.BUTTON_TITLE_IS_STRING * Components.interfaces.nsIPrompt.BUTTON_POS_2) +
          Components.interfaces.nsIPrompt.BUTTON_POS_2_DEFAULT;
        break;
      case "chooseOpen":
        ...
        buttonFlags +=
          (Components.interfaces.nsIPrompt.BUTTON_TITLE_IS_STRING * Components.interfaces.nsIPrompt.BUTTON_POS_2) +
          Components.interfaces.nsIPrompt.BUTTON_POS_0_DEFAULT;
        break;
      default:
        Components.utils.reportError("Unexpected dialog type: " + dialogType);
        return "cancel";
    }
    ...
    Services.ww.registerNotification(function onOpen(subj, topic) {
      if (topic == "domwindowopened" && subj instanceof Components.interfaces.nsIDOMWindow) {
...
DownloadsDataCtor.prototype = {
  ...
  playDownloadSound: function()
  {
    if (Services.prefs.getBoolPref("browser.download.finished_download_sound")) {
      if (!this._sound)
        this._sound = Cc["@mozilla.org/sound;1"].createInstance(Components.interfaces.nsISound);
      try {
